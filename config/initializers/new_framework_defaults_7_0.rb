#このファイルは、Rails 7.0アップグレード時にフレームワークのデフォルト設定を変更するための設定ファイルです。
#Railsアプリケーションを新しいバージョンにアップグレードする際、一部のデフォルト設定が変更されることがあります。
#このファイルは、それらの変更を段階的に適用することを容易にするためのもので、各設定項目はコメントアウトされた状態で提供されます。
#アプリケーションが新しいデフォルトで正しく動作することを確認できたら、個々の設定をコメントアウトから解除し、
#最終的にはconfig.load_defaultsを7.0に設定してこのファイルを削除することが推奨されます。
